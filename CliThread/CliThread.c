...
//CLI for temp/humidity sensor
static const CLI_Command_Definition_t xTempHumidity = {"temphum", "temphum: Returns a value from the Temperature and Humidity sensor\r\n", (const pdCOMMAND_LINE_CALLBACK)CLI_GetTHData, 0};	
...
void vCommandConsoleTask(void *pvParameters)
{
	...
	FreeRTOS_CLIRegisterCommand(&xTempHumidity);
...
BaseType_t CLI_GetTHData( int8_t *pcWriteBuffer,size_t xWriteBufferLen,const int8_t *pcCommandString )
{
	...
	int error = SHTC3_Measure(temperature,humidity);
	
	if(error != ERROR_NONE)
	{
		humidPacket.humidity = rand() % 100;
		tempPacket.temperature = rand() % 100;
	}
	else
	{
		humidPacket.humidity = humidity;
		tempPacket.temperature = temperature;
		char buffer_curr[64];
		sprintf(buffer_curr, "Humidity 1= %d\n",humidity);
		SerialConsoleWriteString(buffer_curr);
		sprintf(buffer_curr, "Temp 2= %d\n",temperature);
		SerialConsoleWriteString(buffer_curr);
	}
	
	
	WifiAddTempDataToQueue( &tempPacket);
	MQTT_HandleTempMessages();
	WifiAddHumidDataToQueue(&humidPacket);
