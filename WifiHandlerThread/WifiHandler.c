...
QueueHandle_t xQueueTempBuffer = NULL;		///< Queue to send Temperature data to the cloud
QueueHandle_t xQueueHumidBuffer = NULL;		///< Queue to send Humidity data to the cloud
...
void SubscribeHandlerTempTopic(MessageData *msgData)
{
	LogMessage(LOG_DEBUG_LVL, "\r\nTemperature topic received!\r\n");
	LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);
}

void SubscribeHandlerHumTopic(MessageData *msgData)
{
	LogMessage(LOG_DEBUG_LVL, "\r\nHumidity topic received!\r\n");
	LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);
}
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
				...
				mqtt_subscribe(module_inst, TEMP_TOPIC, 2, SubscribeHandlerTempTopic);
				mqtt_subscribe(module_inst, HUMID_TOPIC, 2, SubscribeHandlerHumTopic);
...
void MQTT_HandleTempMessages(void)
{
	struct TemperaturePacket TempPacket;
	if (pdPASS == xQueueReceive(xQueueTempBuffer, &TempPacket, 0)) {
		snprintf(mqtt_msg, 63, "%d", TempPacket.temperature);
		mqtt_publish(&mqtt_inst, TEMP_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}
}

void MQTT_HandleHumidMessages(void)
{
	struct HumidityPacket HumidPacket;
	if (pdPASS == xQueueReceive(xQueueHumidBuffer, &HumidPacket, 0)) {
		snprintf(mqtt_msg, 63, "%d", HumidPacket.humidity);
		mqtt_publish(&mqtt_inst, HUMID_TOPIC, mqtt_msg, strlen(mqtt_msg), 1, 0);
	}
}
...
void vWifiTask(void *pvParameters)
{
	...
	xQueueTempBuffer = xQueueCreate(10, sizeof(struct TemperaturePacket));
	xQueueHumidBuffer = xQueueCreate(10, sizeof(struct HumidityPacket));
    xQueueDistanceBuffer = xQueueCreate(5, sizeof(uint16_t));

    if (xQueueWifiState == NULL || xQueueImuBuffer == NULL || xQueueGameBuffer == NULL || xQueueDistanceBuffer == NULL || xQueueHumidBuffer == NULL || xQueueTempBuffer == NULL) {
...
/**
 void WifiAddTempDataToQueue(struct TemperaturePacket *tempPacket)
 * @brief	Adds a Temperature struct to the queue to send via MQTT
 * @param[out]
...
*/
int WifiAddTempDataToQueue(struct TemperaturePacket *tempPacket)
{
    int error = xQueueSend(xQueueTempBuffer, tempPacket, (TickType_t)10);
    return error;
}

/**
 void WifiAddHumidDataToQueue(struct HumidityPacket *humidPacket)
 * @brief	Adds a Humid struct to the queue to send via MQTT
 * @param[out]
...
*/
int WifiAddHumidDataToQueue(struct HumidityPacket *humidPacket)
{
    int error = xQueueSend(xQueueHumidBuffer, humidPacket, (TickType_t)10);
    return error;
}
